...
mod tests {
    ...
    #[test]
    fn test_antigravity_rules_parsing() {
        let v = serde_json::json!({
            "permissions": {
                "allow": ["command(git *)", "command(cargo test)"],
                "deny": ["command(rm -rf *)"],
                "ask": ["command(npm *)"]
            }
        });
        ...
        assert_eq!(ask, vec!["npm *"]);
        assert_eq!(allow, vec!["git *", "cargo test"]);
        assert_eq!(
            check_command_with_rules("git status", &deny, &ask, &allow),
            PermissionVerdict::Allow
        );
        assert_eq!(
            check_command_with_rules("cargo test", &deny, &ask, &allow),
            PermissionVerdict::Allow
        );
        assert_eq!(
            check_command_with_rules("npm run build", &deny, &ask, &allow),
            PermissionVerdict::Ask
        );
        assert_eq!(
            check_command_with_rules("rm -rf src", &deny, &ask, &allow),
            PermissionVerdict::Deny
        );
        assert_eq!(
            check_command_with_rules("python app.py", &deny, &ask, &allow),
            PermissionVerdict::Default
        );
    }
}
